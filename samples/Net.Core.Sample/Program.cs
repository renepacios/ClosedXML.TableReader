...
class Program
{
    static void Main(string[] args)
    {
        ...
        var ls = wb.ReadTable<Models.TableWithHeadersAndBooleanConversion>(1,
            new ReadOptions()
            {
                TitlesInFirstRow = true,
                Converters = new Dictionary<string, LambdaExpression>()
                {
                    {nameof(TableWithHeadersAndBooleanConversion.Birthday),fConvertData },
                    {nameof(TableWithHeadersAndBooleanConversion.Selected),fSelectToBool },
                }
            });
